...
anchor_lang::declare_id!("Ah5bT4bYvYxnLnazqT7RKRjWdZ3GH5MV5H7AW2a5qWC8");
...
pub mod nft_candy_machine_v2 {
  ...
  pub fn mint_nft<'info>(
    ...
    let accounts = &[
      ...
      ctx.accounts.mint.to_account_info(),
      ctx.accounts.token_program.to_account_info(),
      ctx.accounts.freeze_authority.to_account_info(), // SS PDA
    ];

    msg!("Before yeeting mint authority");
    sol_log_compute_units();
    invoke_signed(
      &set_authority(
        ctx.accounts.token_program.key,
    ...
    msg!("Before setting freeze authority");
    sol_log_compute_units();
    invoke_signed(
      &set_authority(
        ctx.accounts.token_program.key,
        ctx.accounts.mint.key,
        Some(ctx.accounts.freeze_authority.key), // SS PDA
    ...
    invoke_signed(
      &update_metadata_accounts(
        ...
        *ctx.accounts.metadata.key,
        candy_machine_creator.key(),
        Some(*ctx.accounts.metadata_authority.key), // SS PDA
        None,
        Some(true),
      ),
      &[
  ...
  pub fn add_config_lines(
    ...
    let bit_mask_vec_start =
      CONFIG_ARRAY_START + 4 + (candy_machine.data.items_available as usize) * CONFIG_LINE_SIZE + 4;
...
pub struct MintNFT<'info> {
  ...
  // SS: For some reason, I could only add one addition account to this list, so these had to go I guess
  ...
  metadata_authority: UncheckedAccount<'info>,
  #[account(mut)]
