...
use {
  ...
  // Use anchor?
  spl_token::{
    instruction::{freeze_account, set_authority, AuthorityType},
    state::Mint,
  },
  std::cell::Ref,
};
anchor_lang::declare_id!("Ab4QV5PD56RUM5Q4u6fHPykornD9ispV1UXNMksC1V2w");
...
pub mod nft_candy_machine {
  ...
  pub fn mint_nft<'info>(ctx: Context<'_, '_, '_, 'info, MintNFT<'info>>) -> ProgramResult {
    ...
    //         *ctx.accounts.mint.key,
    //         candy_machine.key(), // Should be BB PDA?
    //         *ctx.accounts.mint_authority.key,
    //         *ctx.accounts.metadata.key,
    //         *ctx.accounts.payer.key,
    ...
    // TODO: Master edition stuff?
    // TODO: Hardcode BB PDAs?
    // TODO: Init BB accounts
    ...
    let accounts = &[
      ...
      ctx.accounts.mint.to_account_info(),
      ctx.accounts.token_program.to_account_info(),
      ctx.accounts.freeze_authority.to_account_info(), // BB PDA
    ];
    ...
    invoke_signed(
      &set_authority(
        ctx.accounts.token_program.key,
        ctx.accounts.mint.key,
        Some(ctx.accounts.freeze_authority.key), // BB PDA
    ...
    invoke_signed(
      &update_metadata_accounts(
        ...
        *ctx.accounts.metadata.key,
        candy_machine.key(),
        Some(*ctx.accounts.metadata_authority.key), // BB PDA
        None,
        Some(true),
      ),
      &[
  ...
  pub fn initialize_config(ctx: Context<InitializeConfig>, data: ConfigData) -> ProgramResult {
    ...
    let new_symbol = config.data.symbol.clone() + std::str::from_utf8(&array_of_zeroes).unwrap();
  ...
  pub fn add_config_lines(
    ...
    let bit_mask_vec_start =
      CONFIG_ARRAY_START + 4 + (config.data.max_number_of_lines as usize) * CONFIG_LINE_SIZE + 4;
...
pub struct MintNFT<'info> {
  ...
  #[account(mut)]
  token: UncheckedAccount<'info>, // ADDED
  mint_authority: Signer<'info>,
  update_authority: Signer<'info>,
  freeze_authority: UncheckedAccount<'info>, // ADDED TODO: FREEZE
  metadata_authority: UncheckedAccount<'info>, // ADDED
  #[account(mut)]
